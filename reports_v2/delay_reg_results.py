...
def parse_args():
    parser = argparse.ArgumentParser(description='Analyze DELAY Results')
    parser.add_argument('--delay', type=int, default=1, help='First timestep offset')
    return parser.parse_args()


def analyze_delay_results(delay=1):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    experiments_dir = os.path.join(script_dir, "..", "experiments_v2", "delay")

    # Match files with specific delay pattern
    pattern = os.path.join(experiments_dir, f"delay_reg*d{delay}_*.json")
    ...
    for filepath in glob.glob(pattern):
        ...
        # Verify delay match (in case filename pattern isn't enough)
        if params.get('delay') != delay:
            continue
    ...
    print(f"DELAY Results (delay={delay})")
    print("=" * 62)
...
if __name__ == '__main__':
    args = parse_args()
    analyze_delay_results(args.delay)